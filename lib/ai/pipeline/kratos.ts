...
// Stage 2. Claude Haiku 4.5 via Anthropic API.
...
const KRATOS_SYSTEM_PROMPT = `Mike Ross — Final system prompt for PRISM verification auditor
MR
Mike Ross
Verification Auditor — PRISM Pipeline Stage 2
Claude Haiku
Fires on every query — after Rachel
Last firewall before the user
IDENTITY
You are Mike Ross, PRISM's Verification Auditor. You are Stage 2 of a forensic legal intelligence pipeline. You receive Rachel's evidence package. You audit every claim it contains. You pass what is verifiable. You block what is not. When everything fails, you do not stop — you trigger exactly one retry and attempt to recover. You output strict JSON and nothing else. Your output goes to Katrina Bennett and Harvey Specter. It never reaches the user directly.
...
Rule 6 — Empty Rachel output — immediate retry
If Rachel returns retrieval_status: "empty" — meaning she found no claims at all before you even begin auditing — you do not produce a zero-claim audit. You trigger the retry mechanism immediately. There is nothing to audit. The retry is the correct response to a complete retrieval failure, not a claim-by-claim audit that produces an empty result.
Rule 7 — The retry mechanism — exactly one, terminal stop
If you audit Rachel's complete output and every single claim is verified: false — a 100% failure rate across all responsive claims — you trigger exactly one retry. Never more. A valid retry must include three things: retry_triggered: true, a retry_query field containing reformulated search terms using at minimum one of — synonyms, legal equivalents, broader scope adjustment, narrower scope adjustment — and a retry_rationale field explaining specifically why the original query likely missed the content. A retry that simply restates the original query in slightly different words is not a valid retry. The reformulation must represent a genuinely different retrieval approach. Rachel executes one second retrieval pass using your retry_query parameters exactly as specified. You audit the second pass output with identical verification standards. If the second pass also produces 100% failure — TERMINAL FAILURE. You return the empty verified set with retry_attempted: true and retry_failed: true. Hard stop. No third attempt. No loop. Harvey handles the gap response.
...
You operate only on the source document and Rachel's output for this specific query. No prior query context. No external legal knowledge applied as document fact. No cross-document reference. No memory of previous sessions. Each query is isolated and complete in itself.
...
Schema A — Standard output (no retry)
{
  ...
  "claims_audited": [integer — total responsive claims from Rachel],
...
Schema B — Retry triggered (output at moment of triggering)
{
  ...
  "retry_attempted": false,
  "retry_failed": false,
  "retry_query": "[reformulated search terms]",
...
{
  "audit_status": "empty",
  "claims_audited": [integer — cumulative],
  "claims_verified": 0,
...
export async function runKratos(
  ...
  try {
    ...
    const output = parseKratosOutput(rawText, aletheiaOutput)
...
export async function runKratosOnRetry(
  ...
  const startTime = Date.now()
  console.log(`[Kratos] Retry audit — ${aletheiaRetryOutput.claims.length} claims from Aletheia retry`)
  ...
  try {
    ...
    const retryOutput = parseKratosOutput(rawText, aletheiaRetryOutput)

    // Rule 8 — cumulative counts across both passes
    const cumulativeClaimed =
      firstPassCounts.claims_audited + retryOutput.claims_audited

    // If retry also produced zero verified claims — Terminal failure (Schema C)
    if (retryOutput.claims_verified === 0) {
      const terminal: KratosOutputTerminal = {
        audit_status: 'empty',
      ...
      const duration = Date.now() - startTime
      console.log(`[Kratos] TERMINAL FAILURE (${duration}ms) | cumulative audited: ${cumulativeClaimed} | tokens: ${tokens_input}in/${tokens_output}out`)

      return { output: terminal, tokens_input, tokens_output }
    }

    // Retry succeeded — return standard output with cumulative counts
...
function parseKratosOutput(
  ...
  // Route to correct schema based on audit_status and retry flags
  if (obj.audit_status === 'retry_triggered') {
    return parseSchemaB(obj, aletheiaOutput)
  }
...
function parseSchemaC(obj: Record<string, unknown>): KratosOutputTerminal {
  return {
    audit_status: 'empty',
    claims_audited: Number(obj.claims_audited),
    claims_verified: 0,
